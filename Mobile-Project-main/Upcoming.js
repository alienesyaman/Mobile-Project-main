...
export default function Upcoming( {navigation} ) {
  ...
  useEffect(() => {
    axios.get("https://api.themoviedb.org/3/movie/upcoming?api_key=8348c82cbf0c00dd0954a8fd1cc70035")
      .then(res => {
        console.log(res);
        setData(res?.data)
      })
    ...
    <View style={styles.container}>
      <View style={styles.containerInside}>
        {data?.results?.map((item, index) => {
          return (
        <View>
          <TouchableOpacity key={index} onPress={() => navigation.navigate("Detail",{ screen:"Detail", params:{items : item} } )} style={styles.card}>
          ...
          <View style={{display:'flex', justifyContent:'center', alignItems:'center', height:60, width:'100%'}}>
              <Text style={{ marginVertical:'auto', width:'95%', textAlign:'center', color:'white', textTransform:'uppercase', fontSize:16, }}>{item?.original_title}</Text>
        </View>
      </TouchableOpacity>
      </View>
          )
